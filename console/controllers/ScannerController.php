...
class ScannerController extends Controller
{
    ...
    public function actionScan($test = false)
    {
        ...
        $extension_to_search = ["jpg", "png", "jpeg", "gif"];
        ...
        $files = array_diff(scandir($dir_scontrini_da_scansionare), [
            "..",
            ".",
        ]);
        $found = false;
        ...
        try {
            ...
            foreach ($files as $file) {
                ...
                if (in_array($ext, $extension_to_search)) {
                                ...
                                foreach ($dpis_scansione as $dpi) {
                                    ...
                                    foreach ($desks_foto as $desk) {
                                        ...
                                        if (
                                            ...
                                            $scontrino_data = json_decode(
                                                $response->content
                                            );

                                            $model_scansione->piva =
                                                $scontrino_data->piva;
                                            $model_scansione->datascontrino =
                                                $scontrino_data->data;
                                            $model_scansione->ndoc =
                                                $scontrino_data->nDoc;
                                            $model_scansione->lista_articoli = json_encode(
                                                $scontrino_data->listarticoli
                                            );
                                            $model_scansione->testo_rw =
                                                $scontrino_data->testoRW;
                                            $model_scansione->save();

                                            $output .= "\n\nData: {$date}\nElaborazione OCR file {$file}:\n\nDettagli:\nDimensioni:{$dimensione}\nModo: {$mode}\nEngine:{$engine}\nDensità:{$dpi}\n\nRisultati scansione:\n{$response->content}\n\n";
                                            // chiamata della funzione logBatchOCROutput dall'helper LoggerHelper
                                            $logger->logBatchOCROutput(
                                                $output,
                                                $log_file
                                            );
                                            if ($test) {
                                            ...
                                            $attempts++;
                                            $cli_out =
                                                "Il server JAVA non risponde o non è possibile elaborare lo scontrino\n";
                                            $logger->logCLIWorks(
                                                $cli_out,
                                                $logcli_file
                                            );
                    ...
                    if ($move_files) {
                        rename(
                            $dir_scontrini_da_scansionare . $file,
                            "{$url}/frontend/web/uploads/elapsed/{$file}"
                        );
                    }
                    $task = 1;
                ...
                if ($attempts == 3) {
                    $oggetto =
                        "Pixelfabrica | Motore OCR - Errore di rete, il server non risponde";
            ...
            if ($count == 0) {
                ...
                $oggetto =
                    "Pixelfabrica | Motore OCR - Elaborazione OCR con CRON completata";
            ...
            //used back-slash for global namespace
            $oggetto =
                "Pixelfabrica | Motore OCR - Errore di rete, il server non risponde";
            $titolo = "Errore di rete, il server non risponde";
            $cli_out = "Errore di connessione:" . $ex . "\n\n";
            $esito =
                "Errore di connessione al server OCR. Elaborazione non riuscita.";
            echo $cli_out;
            $logger->logCLIWorks($cli_out, $logcli_file);
            if ($mail) {
                $mailer->inviaMail($oggetto, $titolo, $esito, $testmail);
